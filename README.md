...
```
              ...
              └───┬────────────┬────┘
                  │            │
     ┌────────────┘            └──────────────┐
     │  Screened Subnet (DMZ)       VyOS Router (LAN)  │
     │  10.0.1.0/24                 Eth3: 192.168.2.1  │
     │                              Eth2: 192.168.1.1  │
     │                                     │
     │                          ┌──────────┘
     │                          │  Internal LAN
  ...
  ┌──┴───────────┐    ┌─────────┴──────────────────────────────┐
  │              │    │                                         │
  │  Win Web     │    │  Win AD/DNS    RHEL Client   Win11      │
  │  Server      │    │  192.168.1.5   192.168.1.101 192.168.1.100
  │  10.0.1.100  │    │                                         │
  │              │    │  RHEL Client                             │
  │  RHEL DB     │    │  192.168.1.100                          │
  │  10.0.1.200  │    │                                         │
  └──────────────┘    └─────────────────────────────────────────┘
```
